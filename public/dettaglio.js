...
(function () {
  ...
  async function loadDetail() {
    var id = getSampleId();
    if (!id) {
      showError("ID campione non specificato.");
      return;
    }
    ...
    try {
      ...
      var campione = campioni.find(function (c) {
        return c.id === id;
      });

      if (!campione) {
        showError("Campione #" + id + " non trovato.");
        return;
      }
  ...
  function showError(message) {
    detailPage.innerHTML =
      ...
      '<div class="empty-state"><div class="empty-state-icon">⚠️</div><h3>Errore</h3><p>' +
      escapeHtml(message) +
      "</p></div>";
  }
  ...
  function metaItem(label, value, palette, modifier, bgImage) {
    ...
    var hasBg = !!bgImage;
    var bgLayer = hasBg
      ? '<div class="metadata-item__bg" style="background-image:url(\'' +
        escapeAttr(bgImage) +
        "');\"></div>" +
    ...
    return (
      '<div class="metadata-item metadata-item--colored' +
      mod +
      withBg +
      '" style="' +
      style +
      ...
      '<div class="metadata-label" style="color:' +
      labelColor +
      ';">' +
      escapeHtml(label) +
      "</div>" +
  ...
  function imageBlock(src, alt, label) {
    var imageSrc = escapeAttr(basePath + safeAssetPath(src));
  ...
  function renderDetail(c, allCampioni) {
    var related = allCampioni
      .filter(function (s) {
        return s.continente === c.continente && s.id !== c.id;
      })
      .slice(0, 4);
    ...
    var descrizione = escapeHtml(c.descrizione);

    var imagesHtml = imageBlock(
      c.immagine,
      "Campione di sabbia: " + c.nome,
    ...
    if (Array.isArray(c.immagini_extra)) {
      c.immagini_extra.forEach(function (img) {
        if (
          img &&
          isUsableImage(img.src) &&
          !microscopeImages.some(function (existing) {
            return existing.src === img.src;
          })
        ) {
    ...
    imagesHtml += microscopeImages
      .map(function (img) {
        return imageBlock(img.src, img.label + ": " + c.nome, img.label);
      })
      .join("");
    var qrImage = microscopeImages[0] || {
      src: isUsableImage(c.immagine) ? c.immagine : "images/coming-soon.jpg",
      label: "📷 Immagine del campione",
    };
    var qrImageNote =
      microscopeImages.length > 0
        ? "Scansiona per aprire l&#39;immagine al microscopio"
        : "Scansiona per aprire l&#39;immagine del campione";
    imagesHtml +=
      '<div class="image-qr"><div class="image-qr-container" id="qr-image"></div><p class="qr-note">' +
      qrImageNote +
      "</p></div>";
    ...
    detailPage.innerHTML =
      '<div class="detail-back"><a href="index.html" class="btn btn-back">← Torna alla collezione</a></div>' +
      '<div class="detail-header" style="position:relative;"><h1>' +
      nome +
      '</h1><p class="detail-header-location">📍 ' +
      provenienza +
      " — " +
      paese +
      ", " +
      ...
      '<div class="detail-description"><h2>Descrizione</h2><p>' +
      descrizione +
      "</p></div>" +
      ...
      metaItem(
        "Bacino / Mare",
        c.bacino,
        FIELD_PALETTE.bacino,
        "bacino",
        basePath + "images/bg-sea.jpg",
      ) +
      ...
      "</div>" +
      (related.length > 0
        ? '<section class="related-section"><h2>Campioni correlati</h2><div class="related-grid">' +
          related
            .map(function (r) {
              var rsc = getSandColorD(r.tipologia);
              var rtc = getContrastTextColorD(rsc);
              var rId = encodeURIComponent(String(r.id));
              var rImage = escapeAttr(basePath + safeAssetPath(r.immagine));
              var fallbackSrc = escapeAttr(basePath + "images/coming-soon.jpg");
              return (
                '<a href="dettaglio.html?id=' +
                rId +
                '" class="card" aria-label="Vai al campione: ' +
                escapeAttr(r.nome) +
                '">' +
                '<div class="card-image-wrapper"><img class="card-image" src="' +
                rImage +
                '" alt="Campione: ' +
                escapeAttr(r.nome) +
                ...
                escapeHtml(r.nome) +
                '</h3><p class="card-location">📍 ' +
                escapeHtml(r.provenienza) +
                "</p>" +
                '<div class="card-meta"><span class="card-tag continent">' +
                escapeHtml(r.continente) +
                '</span><span class="card-tag">' +
                escapeHtml(r.paese) +
                "</span>" +
                '<li class="card-tag type sand-type-item" tabindex="0" role="listitem" data-sand-color="' +
                rsc +
                '" data-sand-text="' +
                rtc +
                ...
                ";color:" +
                rtc +
                ';transition:all 0.3s ease;"><span class="sand-swatch" style="background-color:' +
                rsc +
                ';transition:background-color 0.3s ease;"></span>' +
                escapeHtml(r.tipologia) +
                "</li>" +
                "</div></div></a>"
              );
            })
    ...
    generateQR(
      ...
      "qr-image",
      140,
      new URL(basePath + safeAssetPath(qrImage.src), window.location.href).href,
    );
