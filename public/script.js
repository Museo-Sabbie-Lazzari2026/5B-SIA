...
function safeAssetPath(src) {
  var value = String(src || "images/coming-soon.jpg")
    .replace(/\\/g, "/")
    .trim();
...
(function () {
  ...
  function updateMsLabel(ms) {
    ...
    if (n === 0) ms.label.textContent = ms.placeholder;
      ...
      ms.label.textContent =
        ms.key === "paese" ? ((COUNTRY_FLAGS[only] || "") + " " + only).trim() : only;
  ...
  async function loadData() {
    try {
      ...
      galleryGrid.innerHTML =
        '<div class="empty-state"><div class="empty-state-icon">⚠️</div><h3>Errore di caricamento</h3><p>Impossibile caricare la collezione. Riprova più tardi.</p></div>';
    }
  }
  ...
  var COUNTRY_CONTINENT = {
    ...
    Andorra: "Europa",
    Angola: "Africa",
    "Antigua e Barbuda": "Nord America",
    "Arabia Saudita": "Asia",
    ...
    Germania: "Europa",
    Ghana: "Africa",
    Giamaica: "Nord America",
    Giappone: "Asia",
    Gibuti: "Africa",
    Giordania: "Asia",
    Grecia: "Europa",
    Grenada: "Nord America",
    Guatemala: "Nord America",
    Guinea: "Africa",
    "Guinea Equatoriale": "Africa",
    "Guinea-Bissau": "Africa",
    Guyana: "Sud America",
    Haiti: "Nord America",
    Honduras: "Nord America",
    India: "Asia",
    Indonesia: "Asia",
    Iran: "Asia",
    Iraq: "Asia",
    ...
    Palau: "Oceania",
    Panama: "Nord America",
    "Papua Nuova Guinea": "Oceania",
    Paraguay: "Sud America",
    Perù: "Sud America",
    Polonia: "Europa",
    Portogallo: "Europa",
    Qatar: "Asia",
    "Regno Unito": "Europa",
    "Rep. Centrafricana": "Africa",
    "Rep. Ceca": "Europa",
    "Rep. del Congo": "Africa",
    "Rep. Dem. del Congo": "Africa",
    "Rep. delle Maldive": "Asia",
    "Rep. di Capo Verde": "Africa",
    "Rep. di Mauritius": "Africa",
    "Repubblica Dominicana": "Nord America",
    Romania: "Europa",
    Ruanda: "Africa",
    Russia: "Europa",
    "Saint Kitts e Nevis": "Nord America",
    "Saint Lucia": "Nord America",
    ...
    Togo: "Africa",
    Tonga: "Oceania",
    "Trinidad e Tobago": "Nord America",
    Tunisia: "Africa",
    Turchia: "Asia",
    Turkmenistan: "Asia",
    Tuvalu: "Oceania",
  ...
  function updatePaesiDropdown() {
    ...
    if (selectedContinenti.size > 0) {
      source = source.filter(function (c) {
        return selectedContinenti.has(c.continente);
      });
    }
    var allPaesi = [
      ...new Set(
  ...
  function renderFilterBadges() {
    ...
    if (search) {
      badges.push(
        '<span class="filter-badge">🔍 ' +
          escapeHtml(search) +
          ' <button class="filter-badge-x" data-filter="search" aria-label="Rimuovi filtro ricerca">✕</button></span>',
      );
    }
    selectedContinenti.forEach(function (v) {
      badges.push(
        '<span class="filter-badge">🌍 ' +
          escapeHtml(v) +
          ' <button class="filter-badge-x" data-filter="continente" data-value="' +
          escapeAttr(v) +
          '" aria-label="Rimuovi continente ' +
          escapeAttr(v) +
          '">✕</button></span>',
      );
    });
    selectedPaesi.forEach(function (v) {
      ...
      badges.push(
        '<span class="filter-badge">' +
          ...
          " " +
          escapeHtml(v) +
          ' <button class="filter-badge-x" data-filter="paese" data-value="' +
          escapeAttr(v) +
          '" aria-label="Rimuovi paese ' +
          escapeAttr(v) +
          '">✕</button></span>',
      );
    });
    selectedTipologie.forEach(function (v) {
      badges.push(
        '<span class="filter-badge">🏖️ ' +
          escapeHtml(v) +
          ' <button class="filter-badge-x" data-filter="tipologia" data-value="' +
          escapeAttr(v) +
  ...
  function renderGallery() {
    ...
    if (shown === 0) {
      galleryGrid.innerHTML =
        '<div class="empty-state"><div class="empty-state-icon">🔍</div><h3>Nessun campione trovato</h3><p>Prova a modificare i filtri o la ricerca per trovare altri campioni.</p></div>';
      removeLoadMoreBtn();
      return;
    }
    ...
    function renderCard(c, i) {
      ...
      var idText = escapeHtml(c.id);
      var nome = escapeHtml(c.nome);
      var nomeAttr = escapeAttr(c.nome);
      var provenienza = escapeHtml(c.provenienza);
      var provenienzaAttr = escapeAttr(c.provenienza);
      var paese = escapeHtml(c.paese);
      var continente = escapeHtml(c.continente);
      var tipologia = escapeHtml(c.tipologia);
      var imageSrc = escapeAttr(basePath + safeAssetPath(c.immagine));
      var fallbackSrc = escapeAttr(basePath + "images/coming-soon.jpg");
      if (currentView === "list") {
        return (
          '<a href="dettaglio.html?id=' +
          id +
          '" class="card card-list" style="animation-delay: ' +
          Math.min(i * 0.03, 0.3) +
          's" aria-label="Scheda dettaglio: ' +
          nomeAttr +
          '">' +
          ...
          paese +
          "</span>" +
          '<span class="card-tag type" style="background-color:' +
          sandColor +
          ";color:" +
          textColor +
          ';">' +
          '<span class="sand-swatch" style="background-color:' +
          sandColor +
          ';"></span>' +
          tipologia +
          "</span>" +
        ...
        return (
          '<a href="dettaglio.html?id=' +
          id +
          '" class="card" style="animation-delay: ' +
          Math.min(i * 0.05, 0.4) +
          's" aria-label="Scheda dettaglio: ' +
          nomeAttr +
          '">' +
          ...
          paese +
          "</span>" +
          '<span class="card-tag type sand-type-item" tabindex="0" role="listitem" data-sand-color="' +
          sandColor +
          '" data-sand-text="' +
          textColor +
  ...
  function updateLoadMoreBtn(totalFiltered) {
    ...
    if (visibleCount >= totalFiltered) {
      container.innerHTML =
        '<p style="color:var(--color-text-muted);font-size:0.92rem;">Hai visto tutti i <strong>' +
        totalFiltered +
        "</strong> campioni</p>";
    } else {
